...
# 업데이트된 형식으로 단일 범주형 변수의 부피를 플롯팅하는 기능
def single_countplot(df, ax, x=None, y=None, top=None, order=True, hue=False, palette='plasma',
    ...
    매개변수
    ----------
    분류자: 사전 형식의 분류자 집합 [dict]
    X : 학습에 사용할 데이터가 담긴 배열 [np.array]
    y: 모델의 대상 벡터가 있는 배열 [np.array]
...
def data_overview(df, corr=False, label_name=None, sort_by='qtd_null', thresh_percent_null=0, thresh_corr_label=0):
    """
    단계:
        1. 세트에 널 데이터가 있는 속성 조사
        2. 각 속성의 원시형 분석
        3. 범주형 속성의 경우 항목 수 분석
        4. 각 속성에 대한 목표와의 Pearson 상관 관계 추출
        5. 인수에 정의된 규칙 적용
        6. 생성된 개요 데이터세트 반환

    인수:
        df -- 분석할 데이터프레임 [pandas.DataFrame]
        label_name -- 대상 변수 이름 [문자열]
        sort_by -- 개요 데이터세트 정렬 열 [string - 기본값: 'qty_null']
        thresh_percent_null -- null 데이터 필터 [int - 기본값: 0]
        threh_corr_label -- 대상 상관 필터 [int - 기본값: 0]

    반환
        df_overview -- 열 분석이 포함된 통합 데이터세트 [pandas.DataFrame]
    """

    # null 데이터 정보를 사용하여 DataFrame 만들기
